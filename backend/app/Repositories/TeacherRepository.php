...
class TeacherRepository {
    ...
    public function all(?string $search = null, ?string $status = null): LengthAwarePaginator {
        return Teacher::with('user.latestInvitation', 'subject')
            ->search($search)
            ->status($status)
            ->latest()
            ->paginate(10);
    ...
    public function find(int $id): ?Teacher {
        return Teacher::with('user.latestInvitation', 'subject')->find($id);
    }

    public function update(Teacher $teacher, array $data): Teacher {
        $teacher->update($data);
        $teacher->user->update($data);
        return $teacher->load('user.latestInvitation', 'subject');
    }
